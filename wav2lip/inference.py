...
def load_model(path):
	...
	if isinstance(checkpoint, torch.jit.ScriptModule):
		# It's a TorchScript model, return it directly
		model = checkpoint.to(device)
		return model.eval()
	else:
		# It's a regular checkpoint
		model = Wav2Lip()
...
def main():
	...
	# Use imageio-ffmpeg executable
	try:
		import imageio_ffmpeg
		ffmpeg_exe = imageio_ffmpeg.get_ffmpeg_exe()
		command = [ffmpeg_exe, '-y', '-i', args.audio, '-i', 'temp/result.avi', '-strict', '-2', '-q:v', '1', args.outfile]
